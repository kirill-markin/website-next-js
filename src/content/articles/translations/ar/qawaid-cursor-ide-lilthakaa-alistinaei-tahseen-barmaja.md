...
## كيفية تكوين قواعد كورسر للحصول على أداء أمثل في الترميز بمساعدة الذكاء الاصطناعي
...
- تفضيل البرمجة الوظيفية على البرمجة الموجهة للكائنات
- استخدم فئات البرمجة الموجهة للكائنات المنفصلة فقط للموصلات والواجهات للأنظمة الخارجية
- اكتب جميع المنطق الآخر بوظائف خالصة (مدخلات/مخرجات واضحة، بدون تغييرات حالة مخفية)
- يجب أن تقوم الوظائف بتعديل قيم الإرجاع فقط - لا تقم أبدًا بتعديل معلمات الإدخال أو الحالة العالمية أو أي بيانات لم يتم إرجاعها صراحة
- قم بإجراء تغييرات صغيرة ومركزة
- اتبع مبادئ DRY و KISS و YAGNI
- استخدم الكتابة الصارمة (إرجاع الدالة، المتغيرات) في جميع اللغات
- استخدم المعلمات المسماة في استدعاءات الدالة عندما يكون ذلك ممكنًا
- لا تكرر الكود؛ تحقق مما إذا كان بعض المنطق مكتوبًا بالفعل قبل كتابته
- تجنب وظائف الغلاف غير الضرورية بدون غرض واضح
- فضّل المجموعات ذات الأنواع القوية على المجموعات العامة عند التعامل مع هياكل البيانات المعقدة
- فكر في إنشاء تعريفات نوع مناسبة لهياكل البيانات غير البسيطة
- الأنواع الأصلية مناسبة لهياكل البيانات البسيطة، ولكن استخدم نماذج مناسبة للمعقدة منها
- حاول تجنب استخدام المتغيرات غير المكتوبة والأنواع العامة حيثما أمكن ذلك
- لا تستخدم أبدًا قيم المعلمات الافتراضية في تعريفات الدالة - اجعل جميع المعلمات واضحة
</cursorrules_code_style>

<cursorrules_error_handling>
- ارفع الأخطاء دائمًا بشكل صريح، ولا تتجاهلها أبدًا بصمت
- إذا حدث خطأ في أي جزء منطقي من الكود، ارفعه فورًا ولا تواصل التنفيذ
- استخدم أنواع أخطاء محددة توضح بوضوح ما الذي حدث بشكل خاطئ
- تجنب معالجات الاستثناءات الشاملة التي تخفي السبب الجذري
- يجب أن تكون رسائل الخطأ واضحة وقابلة للتنفيذ
- سجّل الأخطاء مع السياق المناسب قبل رفعها
</cursorrules_error_handling>

<cursorrules_python_specifics>
- فضّل Pydantic على TypedDict لنماذج البيانات (مثلاً، `class ContactData(BaseModel): ...`)
- تجنب `Any` و`@staticmethod`
- استخدم `pyproject.toml` بدلاً من `requirements.txt` عندما يكون ذلك ممكنًا
- للهياكل المعقدة، تجنب المجموعات العامة مثل `List[Dict[str, Any]]`
- ارفع استثناءات محددة مثل `ValueError` أو `TypeError` بدلاً من `Exception` العام
- استخدم الفئات فقط للعملاء الذين يتصلون بأنظمة خارجية (مثلاً، `NotionClient`)
- للمنطق التجاري، استخدم دوال نقية مع العميل كمعلمة أولى: `def change(notion_client: NotionClient, param1: str, param2: int) -> Result:`
</cursorrules_python_specifics>

<cursorrules_typescript_specifics>
- فضّل الواجهات على أسماء الأنواع المستعارة لأشكال الكائنات المعقدة
- استخدم كائنات مكتوبة لإدارة الحالة المعقدة
- استخدم كائنات الخطأ مع رسائل وصفية: `throw new Error('رسالة محددة')`
- استفد من الاتحادات المميزة لسيناريوهات الأنواع المعقدة
</cursorrules_typescript_specifics>

<cursorrules_libraries_and_dependencies>
- قم بالتثبيت في بيئات افتراضية، وليس عالميًا
- أضف إلى تكوينات المشروع، وليس التثبيتات الفردية
- استخدم استكشاف الكود المصدري للفهم
- فضّل إدارة التبعيات على مستوى المشروع على تثبيت الحزمة الفردية:
  - جيد: `pip install -r requirements.txt`
  - أفضل: استخدم `pyproject.toml` مع حزم بايثون الحديثة
- عند إضافة التبعيات، قم بتحديث ملف تكوين المشروع المناسب، وليس البيئة فقط
</cursorrules_libraries_and_dependencies>

<cursorrules_terminal_usage>
- قم بتشغيل `date` للمهام المتعلقة بالتاريخ
- استخدم GitHub CLI مع `printf` للنص متعدد الأسطر:
  `git commit -m "$(printf "العنوان\n\n- النقطة 1\n- النقطة 2")"`
- استخدم دائمًا أوامر git diff غير التفاعلية مع: `git --no-pager diff` أو `git diff | cat`. لا تستخدم `git diff` أو `git diff --cached`.
- فضّل دائمًا الأوامر ذات المعلمات التي لا تتطلب تفاعل المستخدم على تلك التفاعلية (استخدم الأعلام، المتغيرات البيئية، أو ملفات التكوين لتجنب المطالبات)
</cursorrules_terminal_usage>

<cursorrules_planning_practices>
- يمكن للمستخدم أن يطلب منك إنشاء خطة لتنفيذ الميزة
- يجب عليك إنشاء دليل مؤقت
- يجب عليك إنشاء ملف markdown بخطة الميزة في الدليل المؤقت
- يجب أن يحتوي ملف خطة الميزة هذا على الأقسام التالية:
  ...
  3. قائمة بجميع الملفات المراد تغييرها مع وصف نصي لما سيتم تغييره (وليس كودًا)
  4. قائمة مرجعية لجميع المهام التي يجب القيام بها بنمط مربعات الاختيار markdown ثنائي المستوى
- يجب أن يكون ملف خطة الميزة هذا بسيطًا ويحتوي فقط على التغييرات الدنيا الأكثر أهمية المتعلقة بالميزة، يمكن وصف جميع التغييرات الإضافية كأفكار في قسم إضافي، ولكن لا يجب تنفيذها إذا لم يطلب المستخدم ذلك
</cursorrules_planning_practices>

<cursorrules_repository_practices>
- اقرأ `README.md` إذا لم يكن هناك ملف `.cursorrules`
- لخّص المشروع قبل العمل عليه
</cursorrules_repository_practices>

<cursorrules_code_changes>
- يجب عليك احترام نمط الكود الحالي وأنماطه إذا لم يحدد المستخدم خلاف ذلك
- يجب عليك اقتراح تغييرات دنيا فقط متعلقة بحوار المستخدم الحالي
- يجب عليك تغيير أقل عدد ممكن من الأسطر مع حل المشكلة
- يجب عليك التركيز فقط على ما يطلبه المستخدم في الحوار الحالي، لا تحسينات إضافية
- يجب عليك فهم قاعدة الشفرة الحالية قبل اقتراح التغييرات
- يجب عليك البدء بقراءة الملفات ذات الصلة وقاعدة الشفرة قبل اقتراح التغييرات
</cursorrules_code_changes>

</cursorrules_instructions_to_the_dialog>
```

![لوحة تكوين قواعد كورسر العالمية في لوحة الإعدادات](/articles/cursor-ide-rules-global.webp)
