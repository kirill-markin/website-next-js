import { notFound } from 'next/navigation';
import Image from 'next/image';
import Link from 'next/link';
import { Metadata } from 'next';
import { getArticleBySlug, getAllArticleSlugsByLanguage, buildArticleConnections, getAllArticles, getRelatedArticlesByTags } from '@/lib/articles';
import { markdownToHtml } from '@/lib/markdown';
import { SUPPORTED_LANGUAGES, getPathSegmentByLanguage, getLocaleForLanguage, DEFAULT_LANGUAGE } from '@/lib/localization';
import ArticleContent from '@/components/ArticleContent';
import ArticleJsonLd from '@/components/ArticleJsonLd';
import SocialShare from '@/components/SocialShare';
import AuthorBlock from '@/components/AuthorBlock';
import styles from '@/app/articles/articles.module.css';

interface ArticlePageProps {
    params: Promise<{
        lang: string;
        articlesSegment: string;
        slug: string;
    }>;
}

// Generate static parameters for all supported languages
export async function generateStaticParams() {
    const params = [];

    // Get articles for each language
    for (const lang of SUPPORTED_LANGUAGES) {
        if (lang === DEFAULT_LANGUAGE) continue; // Skip default language (handled by /articles/[slug])

        const slugs = await getAllArticleSlugsByLanguage(lang);
        const articlesSegment = getPathSegmentByLanguage('articles', lang);

        for (const slug of slugs) {
            params.push({
                lang,
                articlesSegment,
                slug,
            });
        }
    }

    return params;
}

export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
    const { lang, slug } = await params;
    const article = await getArticleBySlug(slug, lang);

    if (!article) {
        return {
            title: 'Article Not Found',
        };
    }

    // Generate language-specific canonical URL
    const articlesSegment = getPathSegmentByLanguage('articles', lang);
    const canonicalUrl = lang === DEFAULT_LANGUAGE
        ? `https://kirill-markin.com/articles/${slug}/`
        : `https://kirill-markin.com/${lang}/${articlesSegment}/${slug}/`;

    // Generate hreflang alternate URLs
    const alternateLanguages: Record<string, string> = {};

    // Get all articles to build connections
    const allArticles = await getAllArticles();
    const translatedArticles = await getAllArticles(lang);
    const connectedArticles = await buildArticleConnections([...allArticles, ...translatedArticles]);

    // Find this article with its connections
    const currentArticle = connectedArticles.find(a =>
        a.slug === slug && a.metadata.language === lang
    );

    // Add original article as English alternate 
    if (lang !== DEFAULT_LANGUAGE && currentArticle?.metadata.originalArticle) {
        const { slug: enSlug } = currentArticle.metadata.originalArticle;
        alternateLanguages['en'] = `https://kirill-markin.com/articles/${enSlug}/`;
    }

    // Add all translations as alternates
    if (currentArticle?.metadata.translations) {
        for (const translation of currentArticle.metadata.translations) {
            if (translation.language !== lang) {
                const pathSegment = getPathSegmentByLanguage('articles', translation.language);
                const transUrl = translation.language === DEFAULT_LANGUAGE
                    ? `https://kirill-markin.com/articles/${translation.slug}/`
                    : `https://kirill-markin.com/${translation.language}/${pathSegment}/${translation.slug}/`;

                alternateLanguages[translation.language] = transUrl;
            }
        }
    }

    return {
        title: `${article.metadata.title} - Kirill Markin`,
        description: article.metadata.description || '',
        keywords: article.metadata.tags,
        authors: [{ name: article.metadata.publisher || 'Kirill Markin' }],
        openGraph: {
            title: article.metadata.title,
            description: article.metadata.description || '',
            type: 'article',
            url: canonicalUrl,
            images: [
                {
                    url: article.metadata.thumbnailUrl || '/articles/placeholder.webp',
                    width: 1200,
                    height: 630,
                    alt: article.metadata.title,
                }
            ],
            locale: getLocaleForLanguage(lang),
            publishedTime: article.metadata.date,
            modifiedTime: article.metadata.lastmod,
            tags: article.metadata.tags,
            siteName: 'Kirill Markin'
        },
        twitter: {
            card: 'summary_large_image',
            title: article.metadata.title,
            description: article.metadata.description || '',
            images: [article.metadata.thumbnailUrl || '/articles/placeholder.webp'],
        },
        alternates: {
            canonical: canonicalUrl,
            languages: alternateLanguages,
        }
    };
}

export default async function LocalizedArticlePage({ params }: ArticlePageProps) {
    const { lang, slug, articlesSegment } = await params;
    const article = await getArticleBySlug(slug, lang);

    if (!article) {
        notFound();
    }

    // Get all articles to build connections (for translations)
    const allArticles = await getAllArticles();
    const translatedArticles = await getAllArticles(lang);
    const connectedArticles = await buildArticleConnections([...allArticles, ...translatedArticles]);

    // Find this article with its connections
    const currentArticle = connectedArticles.find(a =>
        a.slug === slug && a.metadata.language === lang
    ) || article;

    const PLACEHOLDER_IMAGE = '/articles/placeholder.webp';
    const htmlContent = await markdownToHtml(article.content);

    // Generate canonical URL and current path
    const currentPath = `/${lang}/${articlesSegment}/${slug}/`;
    const canonicalUrl = lang === DEFAULT_LANGUAGE
        ? `https://kirill-markin.com/articles/${slug}/`
        : `https://kirill-markin.com${currentPath}`;

    // Get related articles based on tags
    const relatedArticles = await getRelatedArticlesByTags(
        article.slug,
        article.metadata.tags,
        lang,
        5
    );

    return (
        <div className={styles.articlePageContainer}>
            <ArticleJsonLd article={currentArticle} url={canonicalUrl} />

            {/* Fixed social share buttons */}
            <SocialShare url={canonicalUrl} title={article.metadata.title} variant="fixed" />

            <article className={styles.articleContainer}>
                <header className={styles.articleHeader}>
                    <div className={styles.articleMeta}>
                        <div className={styles.metaRow}>
                            <div className={styles.articleTags}>
                                {article.metadata.tags && article.metadata.tags.length > 0 && (
                                    article.metadata.tags.map((tag) => (
                                        <Link key={tag} href={`/${lang}/${articlesSegment}/?tag=${tag}`}>
                                            <span className={styles.tag}>
                                                {tag}
                                            </span>
                                        </Link>
                                    ))
                                )}
                            </div>

                            {article.metadata.date && (
                                <time className={styles.articleDate} dateTime={article.metadata.date}>
                                    {new Date(article.metadata.date).toLocaleDateString(lang === 'en' ? 'en-US' : lang, {
                                        year: 'numeric',
                                        month: 'long',
                                        day: 'numeric',
                                    })}
                                </time>
                            )}
                        </div>
                    </div>
                </header>

                <ArticleContent
                    htmlContent={htmlContent}
                    className={styles.articleContent}
                />

                {/* Related Articles Section */}
                {relatedArticles.length > 0 && (
                    <div className={styles.relatedArticlesSection}>
                        <h2 className={styles.relatedArticlesTitle}>
                            {relatedArticles.every(relatedArticle =>
                                relatedArticle.metadata.tags.some(tag => article.metadata.tags.includes(tag))
                            )
                                ? lang === 'es' ? 'Artículos Relacionados' : 'Related Articles'
                                : lang === 'es' ? 'Artículos Relacionados y Recientes' : 'Related & Recent Articles'
                            }
                        </h2>

                        <div className={styles.mediaMentions}>
                            {relatedArticles.map((relatedArticle, index) => {
                                const isVideo = relatedArticle.metadata.isVideo ||
                                    relatedArticle.metadata.type?.toLowerCase() === 'video';

                                return (
                                    <article
                                        key={relatedArticle.slug}
                                        className={`${styles.mediaMention} ${isVideo ? styles.video : ''}`}
                                    >
                                        <Link
                                            href={`/${lang}/${articlesSegment}/${relatedArticle.slug}/`}
                                            className={styles.mentionLink}
                                        >
                                            <div className={styles.language}>
                                                <div className={styles.text}>[{relatedArticle.metadata.language || lang}]</div>
                                            </div>

                                            {relatedArticle.metadata.type && (
                                                <div className={styles.type}>
                                                    <div className={styles.text}>[{relatedArticle.metadata.type}]</div>
                                                </div>
                                            )}

                                            <div className={styles.thumbnailContainer}>
                                                <Image
                                                    className={styles.thumbnail}
                                                    src={relatedArticle.metadata.thumbnailUrl || PLACEHOLDER_IMAGE}
                                                    alt={relatedArticle.metadata.title}
                                                    width={640}
                                                    height={360}
                                                    sizes="(max-width: 640px) 320px, (max-width: 1200px) 640px, 640px"
                                                    quality={75}
                                                    priority={index < 2}
                                                />
                                            </div>

                                            <div className={styles.content}>
                                                <h3 className={styles.title}>{relatedArticle.metadata.title}</h3>

                                                {relatedArticle.metadata.description && (
                                                    <p className={styles.description}>{relatedArticle.metadata.description}</p>
                                                )}

                                                <div className={styles.footer}>
                                                    <div className={styles.date}>
                                                        {relatedArticle.metadata.date && (
                                                            <time dateTime={relatedArticle.metadata.date}>
                                                                {new Date(relatedArticle.metadata.date).toLocaleDateString(lang === 'en' ? 'en-US' : lang, {
                                                                    year: 'numeric',
                                                                    month: 'long',
                                                                    day: 'numeric',
                                                                })}
                                                            </time>
                                                        )}
                                                    </div>

                                                    {relatedArticle.metadata.achievementValue && relatedArticle.metadata.achievementLabel && (
                                                        <div className={styles.achievement}>
                                                            <div className={styles.value}>{relatedArticle.metadata.achievementValue}</div>
                                                            <div className={styles.label}>{relatedArticle.metadata.achievementLabel}</div>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        </Link>
                                    </article>
                                );
                            })}
                        </div>
                    </div>
                )}

                {/* Single AuthorBlock instance controlled by CSS */}
                <AuthorBlock />

                {/* Mobile social share buttons */}
                <div className={styles.mobileShareContainer}>
                    <h3 className={styles.shareTitle}>
                        {lang === 'es' ? 'Comparte este artículo' : 'Share this article'}
                    </h3>
                    <SocialShare url={canonicalUrl} title={article.metadata.title} variant="inline" />
                </div>
            </article>
        </div>
    );
} 